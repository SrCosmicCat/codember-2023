...
// 1. Convertir a array y a minúsculas
const ARRAY = STRING.toLowerCase().split(' ')

// 2. Array para almacenar resultados
let resObj = []

// 3. Obtener los elementos únicos
ARRAY.forEach(element => {
    ...
    resObj.push(
        {
            'word' : element,
            'reps' : ARRAY.filter(word => word === element).length
        }
    )
})

// 4. Imprimir con el formato
let printString = ''

resObj.forEach(obj => {
    printString += `${obj.word}${obj.reps}`
})
